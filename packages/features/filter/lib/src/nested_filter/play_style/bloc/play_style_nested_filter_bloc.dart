...
class PlayStyleNestedFilterBloc
  ...
  PlayStyleNestedFilterBloc(
      @factoryParam PlayStyleNestedFilterPageParams playStyleNestedFilterPageParams,
  ...
  void _selectItem(PlayStyle item, Emitter<PlayStyleNestedFilterState> emit) {
    final selectedPlayStyles = List<PlayStyle>.from(
      state.selectedPlayStyles ?? state.playStyleNestedFilterPageParams?.items ?? [],
    ).toList();
    if (selectedPlayStyles.contains(item)) {
      emit(state.copyWith(selectedPlayStyles: selectedPlayStyles..remove(item)));
    } else {
  ...
  void _done() {
    _navigator.closeAny<List<PlayStyle>?>(
      (state.selectedPlayStyles?.isEmpty ?? true) ? null : state.selectedPlayStyles,
    );
  }
}